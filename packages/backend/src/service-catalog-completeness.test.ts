...
const serviceListArbitrary = fc
  .array(entityArbitrary, { minLength: 1, maxLength: 50 })
  .map(services => {
      ...
      fc.asyncProperty(serviceListArbitrary, async services => {
        ...
        for (const displayedService of displayedServices) {
          ...
          const originalService = services.find(
            s => s.metadata.name === displayedService.name,
          );
          ...
          expect(displayedService.hasCompleteMetadata).toBe(
        ...
        for (const registeredService of registeredServices) {
          ...
          expect(correspondingDisplay!.name).toEqual(
            registeredService.metadata.name,
          );
          expect(correspondingDisplay!.description).toEqual(
            registeredService.metadata.description,
          );
          expect(correspondingDisplay!.owner).toEqual(
            registeredService.spec?.owner,
          );
          expect(correspondingDisplay!.type).toEqual(
            registeredService.spec?.type,
          );
          expect(correspondingDisplay!.lifecycle).toEqual(
            registeredService.spec?.lifecycle,
          );
        }
    ...
    await fc.assert(
      fc.asyncProperty(serviceListArbitrary, async services => {
        ...
        for (const displayedService of displayedServices) {
          ...
          const originalService = minimalServices.find(
            s => s.metadata.name === displayedService.name,
          );
          const expectedCompleteness = !!(
            originalService?.metadata.name &&
    ...
    await fc.assert(
      fc.asyncProperty(serviceListArbitrary, async services => {
        ...
        for (const firstBatchService of firstBatch) {
          const stillPresent = finalDisplay.find(
            d => d.name === firstBatchService.metadata.name,
          );
        ...
        for (const secondBatchService of secondBatch) {
          const nowPresent = finalDisplay.find(
            d => d.name === secondBatchService.metadata.name,
          );
