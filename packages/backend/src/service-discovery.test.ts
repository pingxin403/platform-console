...
class ServiceDiscoveryAutomation {
  ...
  wasDiscoveredWithinTimeLimit(
    ...
    const timeDiffMinutes =
      (now.getTime() - discoveryTime.getTime()) / (1000 * 60);
  ...
  private async fetchCatalogInfo(
    repo: GitHubRepository,
  ): Promise<CatalogInfoFile | null> {
...
const repositoryListArbitrary = fc.array(repositoryArbitrary, {
  minLength: 1,
  maxLength: 20,
});
    ...
    await fc.assert(
      fc.asyncProperty(repositoryListArbitrary, async repositories => {
        ...
        const discoveredEntities =
          await freshServiceDiscovery.scanGitHubRepositories(repositories);
        ...
        const registeredServices =
          freshServiceDiscovery.getDiscoveredServices();
        ...
        for (const entity of discoveredEntities) {
          ...
          expect(
            freshServiceDiscovery.wasDiscoveredWithinTimeLimit(
        ...
        for (const entity of discoveredEntities) {
          const foundService = registeredServices.find(
            s => s.metadata.name === entity.metadata.name,
          );
  ...
  it('should handle repositories without catalog-info.yaml files gracefully', async () => {
    await fc.assert(
      fc.asyncProperty(repositoryListArbitrary, async repositories => {
        ...
        expect(registeredServices.length).toBeLessThanOrEqual(
          repositories.length,
        );
