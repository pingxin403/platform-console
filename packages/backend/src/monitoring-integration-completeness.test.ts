...
class MockMonitoringService {
  async getMonitoringIntegration(entity: Entity): Promise<MonitoringIntegration> {
    ...
    if (annotations['datadog/dashboard-url']) {
      ...
      integration.datadogDashboard = {
        url: dashboardUrl,
        embedded: true,
        serviceFiltered: dashboardUrl.includes(`service%3A${entity.metadata.name}`)
      };
    }
    ...
    if (annotations['datadog/dashboard-url'] || annotations['sentry/project-slug']) {
      ...
      if (hasActiveAlert) {
        integration.alertStatus = {
          severity: fc.sample(fc.constantFrom('low', 'medium', 'high', 'critical'), 1)[0] as any,
          active: true,
  ...
  async checkRBACPermissions(userId: string, service: string): Promise<boolean> {
  ...
  test('Property 9: Monitoring integration completeness', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.record({
          ...
          metadata: fc.record({
            ...
            annotations: fc.option(
              fc.record({
                'datadog/dashboard-url': fc.option(fc.constant(''), { freq: 1 }), // Will be set based on service name
                'sentry/project-slug': fc.option(fc.constant(''), { freq: 1 }), // Will be set based on service name
              }),
          ...
          if (entity.metadata.annotations) {
            ...
            if (hasDatadog) {
              entity.metadata.annotations['datadog/dashboard-url'] = 
                `https://app.datadoghq.com/dashboard/list?q=service%3A${entity.metadata.name}`;
            }
            
            if (hasSentry) {
              entity.metadata.annotations['sentry/project-slug'] = 
        ...
        async (entity: Entity) => {
          const integration = await monitoringService.getMonitoringIntegration(entity);
          const annotations = entity.metadata.annotations || {};
          ...
          if (annotations['datadog/dashboard-url']) {
            ...
            expect(integration.datadogDashboard!.url).toBe(annotations['datadog/dashboard-url']);
          ...
          if (annotations['sentry/project-slug']) {
            ...
            integration.sentryErrors.forEach(error => {
              ...
              expect(['resolved', 'unresolved', 'ignored']).toContain(error.status);
          ...
          if (integration.alertStatus?.active) {
            ...
            expect(['low', 'medium', 'high', 'critical']).toContain(integration.alertStatus.severity);
          ...
          const hasMonitoringAnnotations = annotations['datadog/dashboard-url'] || annotations['sentry/project-slug'];
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ userId, serviceName }) => {
          const hasPermission = await monitoringService.checkRBACPermissions(userId, serviceName);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (entity: Entity) => {
          ...
          try {
            integration = await monitoringService.getMonitoringIntegration(entity);
          } catch (error) {
