...
class MockDocumentationSearch implements DocumentationSearch {
    ...
    for (const service of services) {
      try {
        ...
        if (!service.documents || service.documents.length === 0) {
          warnings.push(
            `No documents found for service: ${service.serviceName}`,
          );
          continue;
        }
  ...
  async searchDocumentation(
    ...
    for (const [serviceName, serviceDoc] of this.searchIndex.entries()) {
      ...
      if (
        ...
        !filters.serviceNames.includes(serviceName)
      ) {
        continue;
      }
      ...
      if (
        filters?.categories &&
        !filters.categories.includes(serviceDoc.category)
      ) {
        continue;
      }
      ...
      if (filters?.dateRange) {
        ...
        if (
          serviceDate < filters.dateRange.start ||
          serviceDate > filters.dateRange.end
        ) {
          continue;
        }
      }
      ...
      if (serviceName.toLowerCase().includes(normalizedQuery)) {
        ...
        facets.owners[serviceDoc.owner] =
          (facets.owners[serviceDoc.owner] || 0) + 1;
        facets.categories[serviceDoc.category] =
          (facets.categories[serviceDoc.category] || 0) + 1;
      }

      // Search within service documents
      for (const doc of serviceDoc.documents) {
        const matchResults = this.searchInDocument(
          doc,
          queryTerms,
          normalizedQuery,
        );
        ...
        if (matchResults.length > 0) {
          ...
          // Update facets
          facets.services[serviceName] =
            (facets.services[serviceName] || 0) + 1;
          facets.owners[serviceDoc.owner] =
            (facets.owners[serviceDoc.owner] || 0) + 1;
          facets.categories[serviceDoc.category] =
            (facets.categories[serviceDoc.category] || 0) + 1;
          facets.contentTypes[doc.type] =
            (facets.contentTypes[doc.type] || 0) + 1;
        }
      }
    }
  ...
  async updateIndex(
    ...
    const oldDocCount =
      this.searchIndex.get(serviceName)?.documents.length || 0;
    ...
    this.indexMetadata = {
      ...
      totalServices: this.searchIndex.size,
      totalDocuments:
        this.indexMetadata.totalDocuments -
        oldDocCount +
  ...
  private searchInDocument(
    ...
    for (const heading of doc.headings || []) {
      ...
      if (
        headingLower.includes(fullQuery) ||
        queryTerms.some(term => headingLower.includes(term))
      ) {
        matches.push({
          snippet: heading,
          score: 80,
    ...
    if (
      contentLower.includes(fullQuery) ||
    ...
    for (const codeBlock of doc.codeBlocks || []) {
      ...
      if (
        codeLower.includes(fullQuery) ||
        queryTerms.some(term => codeLower.includes(term))
      ) {
        matches.push({
          snippet: `\`\`\`${codeBlock.language}\n${codeBlock.content.substring(
            0,
            100,
          )}...\n\`\`\``,
          score: 40,
...
class MockFeishuMigration implements FeishuMigration {
  ...
  async scanFeishuWorkspace(workspaceId: string): Promise<FeishuDocument[]> {
    ...
    for (let i = 0; i < documentCount; i++) {
      documents.push({
        ...
        title: `Document ${i + 1}`,
        content: `# Document ${i + 1}\n\nThis is content from Feishu document ${
          i + 1
        }.\n\n## Section 1\nSome content here.\n\n## Section 2\nMore content here.`,
  ...
  async migrateDocument(
    ...
    const startTime = new Date();
    const migrationId = `migration-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    try {
      ...
      if (!document.id || !targetService) {
        ...
        return this.createFailedMigrationResult(
          migrationId,
          document.id,
          targetService,
          startTime,
      ...
      const fileName = `docs/${document.title
        .replace(/[^a-zA-Z0-9-]/g, '-')
        .toLowerCase()}.md`;
      ...
      if (document.attachments.length > 0) {
        for (const attachment of document.attachments) {
          ...
          warnings.push(
            `Attachment ${attachment.name} needs manual download from ${attachment.url}`,
          );
        }
      }

      // Map permissions
      const permissionsMapped = this.mapPermissions(
      ...
      if (document.documentType === 'sheet') {
        warnings.push(
          'Spreadsheet content may require manual formatting adjustment',
        );
      } else if (document.documentType === 'presentation') {
        warnings.push(
          'Presentation content converted to markdown may lose formatting',
        );
      }

      const endTime = new Date();
      const migrationDuration = Math.max(
        1,
  ...
  async validateMigration(migrationId: string): Promise<ValidationResult> {
    ...
    const contentIntegrity =
      migration.success && migration.convertedFiles.length > 0;
    if (!contentIntegrity) {
...
const documentationContentArbitrary = fc
  .string({ minLength: 50, maxLength: 2000 })
  .map(content => {
    const lines = [
...
const documentationFileArbitrary = fc
  .record({
    path: fc
      .string()
...
const serviceDocumentationArbitrary = fc.record({
  ...
  documents: fc.array(documentationFileArbitrary, {
    minLength: 1,
    maxLength: 5,
  }),
  lastUpdated: fc
    .integer({ min: 1577836800000, max: 1735689600000 })
    .map(timestamp => new Date(timestamp)),
  tags: fc.array(fc.string({ minLength: 3, maxLength: 10 }), {
    minLength: 1,
    maxLength: 5,
  }),
...
const serviceDocumentationListArbitrary = fc
  .array(serviceDocumentationArbitrary, { minLength: 1, maxLength: 10 })
  .map(services => {
...
const feishuAttachmentArbitrary = fc.record({
  ...
  url: fc
    .string()
    .map(s => `https://feishu.com/attachments/${s.substring(0, 16)}`),
});
...
const feishuDocumentArbitrary = fc.record({
  ...
  permissions: fc.array(feishuPermissionArbitrary, {
    minLength: 1,
    maxLength: 5,
  }),
  lastModified: fc
  ...
  attachments: fc.array(feishuAttachmentArbitrary, {
    minLength: 0,
    maxLength: 3,
  }),
});
...
describe('Documentation Search and Migration', () => {
  ...
  it('should index and search across all service documentation', async () => {
    await fc.assert(
      fc.asyncProperty(
        serviceDocumentationListArbitrary,
        ...
        async (services, query) => {
          ...
          const expectedDocCount = services.reduce(
            (total, service) => total + service.documents.length,
            0,
          );
          ...
          for (const result of searchResult.results) {
            ...
            expect(['title', 'content', 'heading', 'code']).toContain(
              result.matchType,
            );
  ...
  it('should support real-time index updates when documentation changes', async () => {
    await fc.assert(
      fc.asyncProperty(
        serviceDocumentationListArbitrary,
        serviceDocumentationArbitrary,
        async (initialServices, updatedService) => {
          // Create a fresh search instance for each property test run
          const freshSearch = new MockDocumentationSearch();

          // Act: Index initial documentation
          const initialIndexResult = await freshSearch.indexDocumentation(
            initialServices,
          );
          ...
          expect(updatedIndex.lastUpdated.getTime()).toBeGreaterThan(
            initialIndex.lastUpdated.getTime(),
          );

          // Assert: Document count should reflect the update
          const wasExistingService = initialServices.some(
            s => s.serviceName === updatedService.serviceName,
          );
          if (wasExistingService) {
            ...
            expect(updatedIndex.totalDocuments).toEqual(
              initialTotalDocs + updatedService.documents.length,
            );
            expect(updatedIndex.totalServices).toEqual(
              initialIndex.totalServices + 1,
            );
          }

          // Assert: Updated service should be searchable
          const searchResult = await freshSearch.searchDocumentation(
            updatedService.serviceName,
          );
          const serviceResults = searchResult.results.filter(
            r => r.serviceName === updatedService.serviceName,
          );
  ...
  it('should support migration from Feishu documents with import capabilities', async () => {
    await fc.assert(
      fc.asyncProperty(
        feishuDocumentArbitrary,
        serviceNameArbitrary,
        async (feishuDoc, targetService) => {
          ...
          const migrationResult = await freshMigration.migrateDocument(
            feishuDoc,
            targetService,
          );
          ...
          if (migrationResult.success) {
            ...
            if (feishuDoc.attachments.length > 0) {
              const attachmentFiles = migrationResult.convertedFiles.filter(f =>
                f.includes('assets/'),
              );
              expect(attachmentFiles.length).toEqual(
                feishuDoc.attachments.length,
              );

              // Should warn about manual attachment download
              const attachmentWarnings = migrationResult.warnings.filter(w =>
                w.includes('Attachment'),
              );
            ...
            if (feishuDoc.documentType === 'sheet') {
              const sheetWarnings = migrationResult.warnings.filter(w =>
                w.includes('Spreadsheet'),
              );
              expect(sheetWarnings.length).toBeGreaterThan(0);
            } else if (feishuDoc.documentType === 'presentation') {
              const presentationWarnings = migrationResult.warnings.filter(w =>
                w.includes('Presentation'),
              );
              expect(presentationWarnings.length).toBeGreaterThan(0);
            }

            // Act: Validate the migration
            const validationResult = await freshMigration.validateMigration(
              migrationResult.migrationId,
            );

            // Assert: Migration validation should work
            expect(validationResult.migrationId).toEqual(
              migrationResult.migrationId,
            );
            ...
            expect(Array.isArray(validationResult.validationWarnings)).toBe(
              true,
            );
            ...
            const rollbackResult = await freshMigration.rollbackMigration(
              migrationResult.migrationId,
            );

            // Assert: Should support rollback
            expect(rollbackResult.migrationId).toEqual(
              migrationResult.migrationId,
            );
            ...
            if (rollbackResult.rolledBack) {
              expect(rollbackResult.filesRemoved).toEqual(
                migrationResult.convertedFiles,
              );
            }
          } else {
  ...
  it('should handle Feishu workspace scanning and bulk migration', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        serviceNameArbitrary,
        async (workspaceId, targetService) => {
          // Create a fresh migration instance for each property test run
          const freshMigration = new MockFeishuMigration();

          // Act: Scan Feishu workspace
          const documents = await freshMigration.scanFeishuWorkspace(
            workspaceId,
          );
          ...
          for (const doc of documents) {
            ...
            expect(['doc', 'sheet', 'presentation']).toContain(
              doc.documentType,
            );
      ...
      fc.asyncProperty(
        ...
        async (services, query) => {
          ...
          if (
            ...
            const serviceFilteredResult = await freshSearch.searchDocumentation(
              query,
              {
            ...
            const ownerFilteredResult = await freshSearch.searchDocumentation(
              query,
              {
                owners: [owners[0]],
              },
            ...
            const categoryFilteredResult =
              await freshSearch.searchDocumentation(query, {
                categories: [categories[0]],
              });

            // Assert: Results should only include services from filtered category
            const categoryServices = services.filter(
              s => s.category === categories[0],
            );
            const categoryServiceNames = categoryServices.map(
              s => s.serviceName,
            );
            ...
            const dateFilteredResult = await freshSearch.searchDocumentation(
              query,
              {
                dateRange: {
                  start: oneMonthAgo,
            ...
            for (const result of dateFilteredResult.results) {
              ...
              expect(service!.lastUpdated.getTime()).toBeGreaterThanOrEqual(
                oneMonthAgo.getTime(),
              );
              expect(service!.lastUpdated.getTime()).toBeLessThanOrEqual(
                now.getTime(),
              );
            }
          }
  ...
  it('should handle edge cases and error conditions gracefully', async () => {
    await fc.assert(
      fc.asyncProperty(serviceDocumentationListArbitrary, async services => {
        ...
        const noIndexResult = await freshSearchInstance.searchDocumentation(
          'test',
        );
        ...
        const invalidValidation = await freshMigration.validateMigration(
          'non-existent-id',
        );
        ...
        const invalidRollback = await freshMigration.rollbackMigration(
          'non-existent-id',
        );
        ...
        const servicesWithoutDocs = services.map(s => ({
          ...s,
          documents: [],
        }));
        const emptyDocsResult = await freshSearch.indexDocumentation(
          servicesWithoutDocs,
        );
