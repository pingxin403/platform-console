...
describe('Multi-Environment Support Property Tests', () => {
      ...
      fc.asyncProperty(
        ...
        async (entity: Entity) => {
          ...
          if (multiStatus) {
            ...
            expect(Object.keys(multiStatus.environments).length).toBeGreaterThan(0);
  ...
  test('Manual sync operations for multi-environment services', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          appName: fc.string({ minLength: 1, maxLength: 50 }).filter(s => /^[a-z0-9-]+$/.test(s)),
        ...
        async ({ appName, environment, syncOptions }) => {
          ...
          const envAppName = environment === 'production' ? 
            `${appName}-prod` : 
            `${appName}-${environment}`;

          // Act: Trigger manual sync operation
          const syncResult = await argocdService.syncApplication(envAppName, syncOptions);
          ...
          if (syncResult.success) {
            ...
            if (syncResult.operation) {
              ...
              expect(syncResult.operation.triggeredBy).toBe(syncOptions.triggeredBy);
              expect(syncResult.operation.options.prune).toBe(syncOptions.prune);
              expect(syncResult.operation.options.dryRun).toBe(syncOptions.dryRun);
              expect(syncResult.operation.options.force).toBe(syncOptions.force);
              expect(syncResult.operation.status).toMatch(/^(pending|in_progress|completed|failed)$/);
            }
          } else {
  ...
  test('Environment-specific application naming consistency', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          baseAppName: fc.string({ minLength: 1, maxLength: 30 }).filter(s => /^[a-z0-9-]+$/.test(s)),
          ...
          if (multiStatus) {
            ...
            Object.entries(multiStatus.environments).forEach(([env, status]) => {
              if (env === 'production') {
                ...
                const isValidProdName = status.applicationName === baseAppName ||
                ...
                const hasEnvSuffix = status.applicationName.includes(env) ||
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ appName, triggeredBy }) => {
          ...
          const syncResult = await argocdService.syncApplication(appName, { triggeredBy });

          if (syncResult.success && syncResult.syncId) {
            // Get operation status
            const operationStatus = await argocdService.getSyncOperationStatus(syncResult.syncId);
            ...
            if (operationStatus) {
              ...
              expect(operationStatus.status).toMatch(/^(pending|in_progress|completed|failed)$/);
              expect(new Date(operationStatus.triggeredAt)).toBeInstanceOf(Date);
            }
          }
        }
      ),
