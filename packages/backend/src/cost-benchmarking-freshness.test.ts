...
class MockCostBenchmarkingService implements CostBenchmarkingService {
  ...
  async benchmarkSimilarServices(serviceName: string): Promise<ServiceBenchmark[]> {
    const benchmarks = this.benchmarkData.get(serviceName) || [];
    
    // Sort by efficiency and calculate percentiles
    const sortedBenchmarks = [...benchmarks].sort((a, b) => a.efficiency - b.efficiency);
  ...
  async getCostDataFreshness(serviceName: string): Promise<CostDataFreshness> {
    ...
    const now = new Date();
    const ageInHours = (now.getTime() - freshness.lastUpdated.getTime()) / (1000 * 60 * 60);
...
const serviceCategoryArbitrary = fc.constantFrom(
  'microservice', 'api', 'frontend', 'database', 'worker', 'cache', 'queue'
);
...
const benchmarkListArbitrary = fc.array(benchmarkArbitrary, { minLength: 2, maxLength: 10 })
  .map(benchmarks => {
    ...
    return uniqueBenchmarks.length >= 2 ? uniqueBenchmarks : [benchmarks[0], benchmarks[1]];
  });
...
describe('Cost Benchmarking and Freshness', () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (targetService, benchmarks) => {
          ...
          const benchmarkResults = await freshService.benchmarkSimilarServices(targetService);
          ...
          const targetResult = benchmarkResults.find(b => b.serviceName === targetService);
          ...
          const percentiles = benchmarkResults.map(b => b.percentile).sort((a, b) => a - b);
      ...
      fc.asyncProperty(
        ...
        async (freshnessDataList) => {
          ...
          for (const freshnessData of freshnessDataList) {
            freshService.setFreshnessData(freshnessData.serviceName, freshnessData);
          }
          
          // Test freshness for each service
          for (const expectedFreshness of freshnessDataList) {
            const actualFreshness = await freshService.getCostDataFreshness(expectedFreshness.serviceName);
            
            // Assert: Freshness data should be complete
            expect(actualFreshness.serviceName).toBe(expectedFreshness.serviceName);
            ...
            const now = new Date();
            const expectedAge = Math.max(0, (now.getTime() - actualFreshness.lastUpdated.getTime()) / (1000 * 60 * 60));
            expect(Math.abs(actualFreshness.dataAge - expectedAge)).toBeLessThan(0.1); // Within 6 minutes tolerance
            ...
            expect(Number.isFinite(actualFreshness.breakdown.memory)).toBe(true);
            expect(Number.isFinite(actualFreshness.breakdown.storage)).toBe(true);
            expect(Number.isFinite(actualFreshness.breakdown.network)).toBe(true);
          }
        }
      ),
      { numRuns: 100 }
  ...
  it('should update cost data and maintain freshness', async () => {
    await fc.assert(
      fc.asyncProperty(
        freshnessDataArbitrary,
        async (initialFreshness) => {
          // Create a fresh service instance for each property test run
          const freshService = new MockCostBenchmarkingService();
          ...
          const beforeUpdate = await freshService.getCostDataFreshness(staleData.serviceName);
          ...
          const afterUpdate = await freshService.getCostDataFreshness(staleData.serviceName);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (category, benchmarks) => {
          ...
          const sameCategoryBenchmarks = benchmarks.map(b => ({ ...b, category }));
          ...
          const sortedByEfficiency = [...benchmarkResults].sort((a, b) => a.efficiency - b.efficiency);
          const sortedByPercentile = [...benchmarkResults].sort((a, b) => a.percentile - b.percentile);
          
          // Services with lower efficiency should have lower percentiles
          for (let i = 0; i < sortedByEfficiency.length; i++) {
            expect(sortedByEfficiency[i].serviceName).toBe(sortedByPercentile[i].serviceName);
          }
