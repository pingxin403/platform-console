...
describe('Development Environment Plugins Integration', () => {
  ...
  describe('DevPod Plugin Configuration', () => {
    it('should have valid DevPod configuration', () => {
      ...
      expect(devpodConfig.getString('api.baseUrl')).toBe('http://localhost:8080');
      
      // Test provisioning configuration
      expect(devpodConfig.getBoolean('provisioning.enabled')).toBe(true);
      expect(devpodConfig.getString('provisioning.defaultProvider')).toBe('docker');
      ...
      expect(templates[0].getString('image')).toBe('mcr.microsoft.com/devcontainers/typescript-node:18');
      
      // Test access configuration
      expect(devpodConfig.getBoolean('access.enabled')).toBe(true);
      expect(devpodConfig.getStringArray('access.methods')).toEqual(['vscode', 'ssh', 'web']);
      ...
      expect(devpodConfig.getBoolean('integration.backstage.enabled')).toBe(true);
      
      // Test UI configuration
      expect(devpodConfig.getBoolean('ui.entityPageIntegration')).toBe(true);
      expect(devpodConfig.getBoolean('ui.showEnvironmentsInCatalog')).toBe(true);
  ...
  describe('Dev Containers Plugin Configuration', () => {
    it('should have valid Dev Containers configuration', () => {
      ...
      // Test API configuration
      expect(devContainersConfig.getString('api.baseUrl')).toBe('http://localhost:8080');
      
      // Test launching configuration
      expect(devContainersConfig.getBoolean('launching.enabled')).toBe(true);
      expect(devContainersConfig.getString('launching.defaultRuntime')).toBe('docker');
      ...
      expect(devContainersConfig.getBoolean('standardization.enabled')).toBe(true);
      const standards = devContainersConfig.getConfig('standardization.standards');
      expect(standards.getConfig('nodejs')).toBeDefined();
      
      const enforcement = devContainersConfig.getConfig('standardization.enforcement');
      ...
      expect(devContainersConfig.getStringArray('sharing.methods')).toEqual(['export', 'registry', 'git']);
      ...
      // Test UI configuration
      expect(devContainersConfig.getBoolean('ui.entityPageIntegration')).toBe(true);
      expect(devContainersConfig.getBoolean('ui.showEnvironmentsInCatalog')).toBe(true);
      expect(devContainersConfig.getString('ui.defaultView')).toBe('containers');
    });
    ...
    it('should validate Dev Containers standardization', () => {
      ...
      expect(nodejsStandard.getString('image')).toBe('mcr.microsoft.com/devcontainers/typescript-node:18');
      expect(nodejsStandard.getStringArray('features')).toContain('ghcr.io/devcontainers/features/git:1');
      expect(nodejsStandard.getStringArray('extensions')).toContain('ms-vscode.vscode-typescript-next');
    });
  });
  ...
  describe('Google Calendar Plugin Configuration', () => {
    it('should have valid Google Calendar configuration', () => {
      ...
      expect(oauth.getString('redirectUri')).toBe('http://localhost:3000/auth/google/callback');
      ...
      expect(workingHours.getStringArray('days')).toEqual(['monday', 'tuesday', 'wednesday', 'thursday', 'friday']);
      ...
      expect(calendarConfig.getStringArray('meetings.types')).toEqual(['meeting', 'appointment', 'event']);
      ...
      expect(availability.getStringArray('statuses')).toEqual(['free', 'busy', 'tentative', 'out_of_office']);
  ...
  describe('Plugin Integration Tests', () => {
    ...
    it('should have proper service annotation configurations', () => {
      ...
      expect(devpodConfig.getString('integration.backstage.annotation')).toBe('devpod.io/template');
    ...
    it('should validate team coordination features', () => {
      ...
      expect(calendarConfig.getBoolean('coordination.enabled')).toBe(true);
      expect(calendarConfig.getBoolean('coordination.dashboard.enabled')).toBe(true);
      expect(calendarConfig.getBoolean('coordination.dashboard.showAvailability')).toBe(true);
      expect(calendarConfig.getBoolean('coordination.dashboard.showUpcomingMeetings')).toBe(true);
  ...
  describe('Configuration Validation', () => {
    ...
    it('should validate API endpoints are properly configured', () => {
      ...
      expect(devContainersConfig.getString('api.baseUrl')).toMatch(/^https?:\/\/.+/);
